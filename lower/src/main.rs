...
fn upper_filename(file_name: &String, file_stem: &String) {
    let lowered_stem = file_stem.to_lowercase();

    let change_result = fs::rename(file_name, file_name.replace(file_stem,&lowered_stem));
...
fn main() {
    ...
    if arguments.len() < 2 {
        println!("Usage: lower filename-to-lower");
        return;
    }
